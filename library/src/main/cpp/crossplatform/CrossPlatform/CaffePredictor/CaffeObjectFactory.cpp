...
shared_ptr<INeuralNetwork>
CCaffeObjectFactory::CreateNeuralNetwork(const string &netName, const string &pathToNetStructure,
                                         const string &pathToNetModel,
    ...
    if (auto objectFactory = _objectFactory.lock()) {
        ...
        if (auto serviceContainer = _serviceContainer.lock()) {
            neuralNetwork = dynamic_pointer_cast<INeuralNetwork>(
                    objectFactory->CreateObject<CCaffeNeuralNetwork>(serviceContainer, netName,
                                                                     pathToNetStructure,
                                                                     pathToNetModel,
                                                                     pathToMeanFile));
        }
...
shared_ptr<INeuralNetworkDatum> CCaffeObjectFactory::CreateNeuralNetworkDatum(const Mat &imageMat,
                                                                              bool needToBeNormalized,
                                                                              bool isColor/* = false*/,
                                                                              const int height/* = 0*/,
                                                                              const int width/* = 0*/) {
    shared_ptr<INeuralNetworkDatum> datum;
    if (auto objectFactory = _objectFactory.lock()) {
        datum = dynamic_pointer_cast<INeuralNetworkDatum>(
                objectFactory->CreateObject<CCaffeDatum>(imageMat, needToBeNormalized, isColor,
                                                         height, width));
    }
...
shared_ptr<INeuralNetworkDatumList> CCaffeObjectFactory::CreateNeuralNetworkDatumList() {
    ...
    if (auto objectFactory = _objectFactory.lock()) {
        datumList = dynamic_pointer_cast<INeuralNetworkDatumList>(
                objectFactory->CreateObject<CCaffeDatumList>());
    }
    return datumList;
}

shared_ptr<INeuralNetworkResult>
CCaffeObjectFactory::CreateNeuralNetworkResult(const vector<pair<int, float>> &rawResult,
                                               int maxIndex, float maxProbability) {
    shared_ptr<INeuralNetworkResult> result;
    if (auto objectFactory = _objectFactory.lock()) {
        result = dynamic_pointer_cast<INeuralNetworkResult>(
                objectFactory->CreateObject<CCaffeResult>(rawResult, maxIndex, maxProbability));
    }
...
shared_ptr<INeuralNetworkResultList> CCaffeObjectFactory::CreateNeuralNetworkResultList() {
    ...
    if (auto objectFactory = _objectFactory.lock()) {
        resultList = dynamic_pointer_cast<INeuralNetworkResultList>(
                objectFactory->CreateObject<CCaffeResultList>());
    }
