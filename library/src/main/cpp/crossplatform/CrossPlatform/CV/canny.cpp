...
void llcv_canny7_precomputed_sobel(Mat src, Mat dst, Mat dx, Mat dy, double low_thresh,
                                   double high_thresh) {
...
//  }
    ...
    for (uint16_t row_index = 0; row_index < image_size.height; row_index++) {
        ...
        for (uint16_t vector_index = 0; vector_index < vector_chunks; vector_index++) {
            ...
            int32x4_t d0_summed = vhaddq_s32(dx_d0_squared,
                                             dy_d0_squared); // halving add to avoid overflow; will compensate for this later
            ...
            float32x4_t approx_root_d0_summed = vmulq_f32(d0_summed_float,
                                                          approx_inv_root_d0_summed);
            float32x4_t approx_root_d1_summed = vmulq_f32(d1_summed_float,
                                                          approx_inv_root_d1_summed);
...
void llcv_adaptive_canny7_precomputed_sobel(Mat src, Mat dst, Mat dx, Mat dy) {
    ...
    double mean =
            (sum_abs_magnitude(dx) + sum_abs_magnitude(dy)) / (src_size.width * src_size.height);
