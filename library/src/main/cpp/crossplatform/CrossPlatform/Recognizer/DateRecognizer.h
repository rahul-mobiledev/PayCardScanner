...
public:
    ...
    virtual shared_ptr<INeuralNetworkResultList>
    Process(cv::Mat &frame, vector<cv::Mat> &samples, cv::Rect &boundingRect);
...
private:
    ...
    void Predict(const vector<cv::Mat> &matrixes,
                 shared_ptr<INeuralNetworkResultList> &neuralNetworkResultList,
                 const shared_ptr<INeuralNetwork> &neuralNetwork);

    void PredictDate(const vector<cv::Mat> &digits,
                     shared_ptr<INeuralNetworkResultList> &neuralNetworkResultList,
                     float &meanConfidence, const shared_ptr<INeuralNetwork> &neuralNetwork);

    bool ValidateDate(const shared_ptr<INeuralNetworkResultList> &dateResult, bool &stop);

    bool RefineDateLocationL0(cv::Mat &dateMat, const vector<cv::Point> dateCenters,
                              vector<cv::Rect> &refinedDateRects);

    bool RefineDateLocationL1(cv::Mat &dateMat, const vector<cv::Point> dateCenters,
                              vector<cv::Rect> &refinedDateRects);
