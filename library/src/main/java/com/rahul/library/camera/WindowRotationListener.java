...
public final class WindowRotationListener {
        ...
        public void register(Context context, Display display, RotationListener listener) {
            ...
            mOrientationListener = new OrientationEventListener(context) {
                ...
                public void onOrientationChanged(int orientation) {
                    if (mDisplay == null)
                        return; // onOrientationChanged() called after unsubscribe. Skip
    ...
    private static class ImplApi17 implements Impl, DisplayManager.DisplayListener {
        ...
        public void onDisplayAdded(int displayId) {
            if (DBG)
                Log.d(TAG, "onDisplayAdded() called with: " + "displayId = [" + displayId + "]");
        }

        @Override
        public void onDisplayRemoved(int displayId) {
            if (DBG)
                Log.d(TAG, "onDisplayRemoved() called with: " + "displayId = [" + displayId + "]");
        }

        @Override
        public void onDisplayChanged(int displayId) {
            if (DBG)
                Log.d(TAG, "onDisplayChanged() called with: " + "displayId = [" + displayId + "]");
