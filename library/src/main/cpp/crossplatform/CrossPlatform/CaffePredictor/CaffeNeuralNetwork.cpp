...
CCaffeNeuralNetwork::CCaffeNeuralNetwork(const shared_ptr<IServiceContainer> &serviceContainer,
                                         const string &name,
                                         const string &pathToNetStructure,
...
bool CCaffeNeuralNetwork::Predict(const shared_ptr<INeuralNetworkDatumList> &datumList,
...
void CCaffeNeuralNetwork::ProcessResult(const caffe::Blob<float> *output,
                                        shared_ptr<INeuralNetworkResultList> &resultList) {
    ...
    for (int n = 0; n < batchSize; n++) {
        ...
        if (auto objectFactory = _objectFactory.lock()) {
            shared_ptr<INeuralNetworkResult> result = objectFactory->CreateNeuralNetworkResult(
                    rawResultPtr, maxIndex, maxValue);
