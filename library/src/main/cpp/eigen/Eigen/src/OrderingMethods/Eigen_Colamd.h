...
// Prototypes of non-user callable routines
    template<typename IndexType>
    static IndexType init_rows_cols(IndexType n_row, IndexType n_col, Colamd_Row<IndexType> Row[],
                                    colamd_col<IndexType> col[], IndexType A[], IndexType p[],
                                    IndexType stats[COLAMD_STATS]);

    template<typename IndexType>
    static void init_scoring(IndexType n_row, IndexType n_col, Colamd_Row<IndexType> Row[],
                             colamd_col<IndexType> Col[], IndexType A[], IndexType head[],
                             double knobs[COLAMD_KNOBS], IndexType *p_n_row2, IndexType *p_n_col2,
                             IndexType *p_max_deg);

    template<typename IndexType>
    static IndexType
    find_ordering(IndexType n_row, IndexType n_col, IndexType Alen, Colamd_Row<IndexType> Row[],
                  colamd_col<IndexType> Col[], IndexType A[], IndexType head[], IndexType n_col2,
    ...
    template<typename IndexType>
    static void detect_super_cols(colamd_col<IndexType> Col[], IndexType A[], IndexType head[],
                                  IndexType row_start, IndexType row_length);

    template<typename IndexType>
    static IndexType
    garbage_collection(IndexType n_row, IndexType n_col, Colamd_Row<IndexType> Row[],
                       colamd_col<IndexType> Col[], IndexType A[], IndexType *pfree);
 ...
 */
    template<typename IndexType>
    static bool colamd(IndexType n_row, IndexType n_col, IndexType Alen, IndexType *A, IndexType *p,
        ...
        {
            stats[COLAMD_STATUS] = COLAMD_ERROR_A_not_present;COLAMD_DEBUG0 (
        ...
        {
            stats[COLAMD_STATUS] = COLAMD_ERROR_p_not_present;COLAMD_DEBUG0 (
        ...
        if (nnz < 0)  /* nnz must be >= 0 */
        {
            ...
            stats[COLAMD_INFO1] = nnz;COLAMD_DEBUG0 (
        ...
        if (need > Alen) {
            ...
            stats[COLAMD_INFO2] = Alen;COLAMD_DEBUG0 (
    ...
    static IndexType init_rows_cols  /* returns true if OK, or false otherwise */
        ...
        for (col = 0; col < n_col; col++) {
            ...
            if ((Col[col].length) < 0) // extra parentheses to work-around gcc bug 10200
            {
                ...
                stats[COLAMD_INFO2] = Col[col].length;COLAMD_DEBUG0 (
        ...
        for (col = 0; col < n_col; col++) {
            ...
            while (cp < cp_end) {
                ...
                if (row < 0 || row >= n_row) {
                    ...
                    stats[COLAMD_INFO3] = n_row;COLAMD_DEBUG0 (
                ...
                if (row <= last_row || Row[row].shared2.mark == col) {
                    ...
                    stats[COLAMD_INFO1] = col;
                    stats[COLAMD_INFO2] = row;
                    (stats[COLAMD_INFO3])++;COLAMD_DEBUG1 (
...
*/
    ...
    static void init_scoring
        ...
        dense_col_count = COLAMD_MAX (0, COLAMD_MIN(knobs[COLAMD_DENSE_COL] * n_row,
                                                    n_row));COLAMD_DEBUG1 (
    ...
    static IndexType find_ordering /* return the number of garbage collections */
        ...
        for (k = 0; k < n_col2; /* 'k' is incremented below */) {
            ...
            if (pivot_row_length > 0) {
                ...
                pivot_row = A[Col[pivot_col].start];COLAMD_DEBUG3 (
                        ("Pivotal row is %d\n", pivot_row));
            } else {
            ...
            while (rp < rp_end) {
                col = *rp++;
                COLAMD_ASSERT (COL_IS_ALIVE(col) && col != pivot_col);COLAMD_DEBUG3 (
                        ("Col: %d\n", col));
                ...
                while (cp < cp_end) {
                    ...
                    if (set_difference == 0) { COLAMD_DEBUG3 (
            ...
            while (rp < rp_end) {
                ...
                if (Col[col].length == 0) { COLAMD_DEBUG4 (
