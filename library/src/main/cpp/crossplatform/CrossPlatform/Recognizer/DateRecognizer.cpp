...
bool CDateRecognizer::Deploy() {
    if (auto factory = _factory.lock()) {
        ...
        _dateRecognitionNeuralNetwork = factory->CreateNeuralNetwork("", _pathDateRecognitionStruct,
                                                                     _pathDateRecognitionModel);
        _dateLocalizationNeuralNetworkL1 = factory->CreateNeuralNetwork("",
                                                                        _pathDateLocalization1Struct,
                                                                        _pathDateLocalization1Model);
        _dateLocalizationNeuralNetworkL0 = factory->CreateNeuralNetwork("",
                                                                        _pathDateLocalization0Struct,
                                                                        _pathDateLocalization0Model);

        return _dateRecognitionNeuralNetwork->IsDeployed() &&
               _dateLocalizationNeuralNetworkL1->IsDeployed() &&
               _dateLocalizationNeuralNetworkL0->IsDeployed() && cascadeFlag &&
               _pathDateLocalizationViola.length() > 0;
    }
...
void CDateRecognizer::Predict(const vector<Mat> &matrixes,
                              shared_ptr<INeuralNetworkResultList> &neuralNetworkResultList,
    ...
    if (auto factory = _factory.lock()) {
        ...
        for (Mat matrix: matrixes) {
            shared_ptr<INeuralNetworkDatum> neuralNetworkDatum = factory->CreateNeuralNetworkDatum(
                    matrix);
...
void CDateRecognizer::PredictDate(const vector<Mat> &digits,
                                  shared_ptr<INeuralNetworkResultList> &neuralNetworkResultList,
...
bool
CDateRecognizer::ValidateDate(const shared_ptr<INeuralNetworkResultList> &dateResult, bool &stop) {
...
bool CDateRecognizer::RefineDateLocationL0(Mat &dateMat, const vector<cv::Point> dateCenters,
                                           vector<cv::Rect> &refinedDateRects) {
    ...
    for (cv::Point center: dateCenters) {
        cv::Rect extendedRect = cv::Rect(center.x - rectSize.width / 2,
                                         center.y - rectSize.height / 2, rectSize.width,
                                         rectSize.height);
        ...
        if (auto factory = _factory.lock()) {
            ...
            cv::Point shift = cv::Point(cvRound(data.at(0).second * 72.0),
                                        cvRound(data.at(1).second * 16.0));
            cv::Rect refinedRect = cv::Rect(shift.x * 2.0 + extendedRect.x,
                                            shift.y * 2.0 + extendedRect.y, 98, 28);
...
bool CDateRecognizer::RefineDateLocationL1(Mat &dateMat, const vector<cv::Point> dateCenters,
                                           vector<cv::Rect> &refinedDateRects) {
    ...
    if (auto factory = _factory.lock()) {
        ...
        for (cv::Point center: dateCenters) {
            ...
            Predict({dateMat(extendedRect)}, neuralNetworkResultList,
                    _dateLocalizationNeuralNetworkL1);

            shared_ptr<INeuralNetworkResult> result = neuralNetworkResultList->GetAtIndex(0);
            vector<pair<int, float>> data = result->GetRawResult();

            // apply refinment to original rects
            cv::Point shift = cv::Point(cvRound(data.at(0).second * 12.0),
                                        cvRound(data.at(1).second * 8.0));
            cv::Rect refinedRect = cv::Rect(extendedRect.x + shift.x, extendedRect.y + shift.y, 98,
                                            28);
...
CDateRecognizer::Process(cv::Mat &frame, vector<cv::Mat> &samples, cv::Rect &boundingRect) {
    ...
    _dateCascade.detectMultiScale(dateMat, dateRects, rejectLevels, levelWeights, 1.01, 1,
                                  0 | CV_HAAR_SCALE_IMAGE, cv::Size(96, 26), cv::Size(100, 30),
                                  true);
    ...
    if (dateRects.size() > rectsLimit) {
        ...
        partial_sort_copy(dateRects.begin(), dateRects.end(), newDateRects.begin(),
                          newDateRects.end(),
                          [&levelWeights, &dateRects](const cv::Rect &r1, const cv::Rect &r2) {
                              auto pos1 = find(dateRects.begin(), dateRects.end(), r1) -
                                          dateRects.begin();
                              auto pos2 = find(dateRects.begin(), dateRects.end(), r2) -
                                          dateRects.begin();
                              if (levelWeights[pos1] != levelWeights[pos2])
                                  return levelWeights[pos1] > levelWeights[pos2];
    ...
    for (cv::Rect &rect: dateRects) {
        cv::Point center = cv::Point(rect.x + rect.width / 2 + dateWindowRect.x,
                                     rect.y + rect.height / 2 + dateWindowRect.y);
    ...
    cv::Rect extendedRect = cv::Rect(dateWindowRect.x - padding, dateWindowRect.y - padding,
                                     dateWindowRect.width + padding * 2,
                                     dateWindowRect.height + padding * 2);
    ...
    if (auto factory = _factory.lock()) {
        for (cv::Rect rect: refinedRects) {
            cv::Point center = cv::Point(rect.x + rect.width / 2,
                                         rect.y + rect.height / 2 + 2); // magic + 2
            ...
            PredictDate(digits, neuralNetworkResultDigits, meanConfidence,
                        _dateRecognitionNeuralNetwork);
