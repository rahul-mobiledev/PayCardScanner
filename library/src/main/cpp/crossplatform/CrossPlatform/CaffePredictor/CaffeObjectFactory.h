...
public:
    ...
    virtual shared_ptr<INeuralNetwork>
    CreateNeuralNetwork(const string &netName, const string &pathToNetStructure,
                        const string &pathToNetModel, const string &pathToMeanFile = "");

    virtual shared_ptr<INeuralNetworkDatum>
    CreateNeuralNetworkDatum(const Mat &imageMat, bool needToBeNormalized = true,
                             bool isColor = false,
                             const int height = 0, const int width = 0);

    virtual shared_ptr<INeuralNetworkDatumList> CreateNeuralNetworkDatumList();

    virtual shared_ptr<INeuralNetworkResult>
    CreateNeuralNetworkResult(const vector<pair<int, float>> &rawResult,
