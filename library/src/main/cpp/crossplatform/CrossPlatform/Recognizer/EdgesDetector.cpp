...
CEdgesDetector::CEdgesDetector(shared_ptr<IFrameStorage> frameStorage) : _lineFlags(
        DetectedLineNoneFlag), _orientation(PayCardsRecognizerOrientationUnknown) {
}
...
const Rect CEdgesDetector::CalcWorkingArea(Size frameSize, int captureAreaWidth,
    ...
    if (orienation == PayCardsRecognizerOrientationPortrait ||
        orienation == PayCardsRecognizerOrientationPortraitUpsideDown) {
    ...
    _internalWindowRect = cv::Rect(windowRect.x - _lineThickness / 2,
                                   windowRect.y - _lineThickness / 2,
                                   windowRect.height + _lineThickness,
                                   windowRect.width + _lineThickness);
    return windowRect;
}

const DetectedLineFlags
CEdgesDetector::DetectEdges(Mat &rawFrame, vector<ParametricLine> &edges, Mat &resultFrame) {
    ...
    cv::Rect topRect = cv::Rect(_lineThickness, 0, _internalWindowRect.width - lineThicknessX2,
                                _lineThickness);
    Mat topEdge = windowMat(topRect);

    cv::Rect bottomRect = cv::Rect(_lineThickness, _internalWindowRect.height - _lineThickness,
                                   _internalWindowRect.width - lineThicknessX2, _lineThickness);
    Mat bottomEdge = windowMat(bottomRect);

    cv::Rect leftRect = cv::Rect(0, _lineThickness, _lineThickness,
                                 _internalWindowRect.height - lineThicknessX2);
    Mat leftEdge = windowMat(leftRect);

    cv::Rect rightRect = cv::Rect(_internalWindowRect.width - _lineThickness, _lineThickness,
                                  _lineThickness, _internalWindowRect.height - lineThicknessX2);
    ...
    if (BestLine(&topLine, topEdge, false)) {
        ...
        if (_orientation == PayCardsRecognizerOrientationPortrait ||
            _orientation == PayCardsRecognizerOrientationPortraitUpsideDown) {
...
ParametricLine
CEdgesDetector::LineByShiftingOrigin(const ParametricLine &oldLine, int xOffset, int yOffset) {
    ...
    double deltaAngle = oldLine.theta - offsetAngle +
                        CV_PI / 2.0f; // because we're working with the line *normal* to theta
