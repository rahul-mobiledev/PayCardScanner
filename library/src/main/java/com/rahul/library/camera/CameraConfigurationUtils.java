    ...
    public static void setBestExposure(Camera.Parameters parameters, final boolean lightOn) {
        ...
        if ((minExposure != 0 || maxExposure != 0) && step > 0.0f) {
            ...
            if (parameters.getExposureCompensation() == compensationSteps) {
                if (DBG)
                    Log.i("CameraConfig", "Exposure compensation already set to " + compensationSteps + " / " + actualCompensation);
            } else {
                if (DBG)
                    Log.i("CameraConfig", "Setting exposure compensation to " + compensationSteps + " / " + actualCompensation);
