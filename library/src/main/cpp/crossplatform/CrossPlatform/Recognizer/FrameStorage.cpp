...
CFrameStorage::CFrameStorage() : _grayOrientation(PayCardsRecognizerOrientationUnknown),
                                 _yuvOrientation(PayCardsRecognizerOrientationUnknown) {
...
bool CFrameStorage::SetRawFrame(const Mat &rawFrame, const vector<ParametricLine> &edges,
                                PayCardsRecognizerOrientation orientation) {
    if (!NormalizeMatrix(rawFrame, edges, _frame)) return false;

    _grayOrientation = orientation;

    if (_grayOrientation != PayCardsRecognizerOrientationPortraitUpsideDown &&
        _grayOrientation != PayCardsRecognizerOrientationPortrait) {
...
bool CFrameStorage::NormalizeMatrix(const Mat &matrix, const vector<ParametricLine> &edges,
                                    Mat &resultMatrix) {
    vector<cv::Point> corners = {cv::Point(0, 0), cv::Point(0, 0), cv::Point(0, 0),
                                 cv::Point(0, 0)};
...
void
CFrameStorage::SetRawY(const void *rawY, const void *rawUV, const vector<ParametricLine> &edges,
                       PayCardsRecognizerOrientation orientation) {
...
void CFrameStorage::NormalizeMatrix(const Mat &src, Mat &dst, int intend,
                                    vector<cv::Point> &corner_points) {
    ...
    llcv_unwarp(src, srcPoints,
                cv::Rect(intend / 2, intend / 2, src.cols - intend, src.rows - intend), dst);
}

bool
CFrameStorage::ParametricIntersect(const ParametricLine &line1, const ParametricLine &line2, int *x,
                                   int *y) {
