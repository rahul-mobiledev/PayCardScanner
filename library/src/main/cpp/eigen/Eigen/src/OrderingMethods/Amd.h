...
/* clear w */
        ...
        static StorageIndex
        cs_wclear(StorageIndex mark, StorageIndex lemax, StorageIndex *w, StorageIndex n) {
            StorageIndex k;
...
/* depth-first search and postorder of a tree rooted at node j */
        template<typename StorageIndex>
        StorageIndex
        cs_tdfs(StorageIndex j, StorageIndex k, StorageIndex *head, const StorageIndex *next,
...
/** \internal
  ...
  * On exit the values of C are destroyed */
        ...
        void minimum_degree_ordering(SparseMatrix<Scalar, ColMajor, StorageIndex> &C,
            ...
            dense = std::max<StorageIndex>(16, StorageIndex(
                    10 * sqrt(double(n))));   /* find dense threshold */
            ...
            {
                if (Cp[i] == -1)
                    k = internal::cs_tdfs<StorageIndex>(i, k, head, next, perm.indices().data(), w);
            }
