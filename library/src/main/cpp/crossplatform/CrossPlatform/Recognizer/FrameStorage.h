...
public:
    ...
    virtual bool SetRawFrame(const Mat &rawFrame, const vector<ParametricLine> &edges,
                             PayCardsRecognizerOrientation orientation);

    virtual bool
    NormalizeMatrix(const Mat &matrix, const vector<ParametricLine> &edges, Mat &resultMatrix);

    virtual void SetRawY(const void *rawY, const void *rawUV, const vector<ParametricLine> &edges,
                         PayCardsRecognizerOrientation orientation);
...
private:
    ...
    bool
    ParametricIntersect(const ParametricLine &line1, const ParametricLine &line2, int *x, int *y);
