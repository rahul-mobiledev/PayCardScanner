...
public:
    ...
    static bool GetInstance(shared_ptr<IRecognitionCoreDelegate> &recognitionDelegate,
                            void *platformDelegate = NULL, void *recognizer = NULL);

    virtual void RecognitionDidFinish(const shared_ptr<IRecognitionResult> &result,
                                      PayCardsRecognizerMode resultFlags) = 0;
