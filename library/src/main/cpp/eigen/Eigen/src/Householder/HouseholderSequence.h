  ...
  */
    ...
    namespace internal {
        ...
        struct hseq_side_dependent_impl {
            ...
            essentialVector(const HouseholderSequenceType &h, Index k) {
                ...
                return Block<const VectorsType, Dynamic, 1>(h.m_vectors, start, k, h.rows() - start,
                                                            1);
            }
        };
    ...
    public:
        ...
        inline void applyThisOnTheRight(Dest &dst) const {
            Matrix<Scalar, 1, Dest::RowsAtCompileTime, RowMajor, 1, Dest::MaxRowsAtCompileTime> workspace(
                    dst.rows());
            ...
            for (Index k = 0; k < m_length; ++k) {
                ...
                dst.rightCols(rows() - m_shift - actual_k)
                        .applyHouseholderOnTheRight(essentialVector(actual_k),
                                                    m_coeffs.coeff(actual_k), workspace.data());
            }
        }
        ...
        inline void applyThisOnTheLeft(Dest &dst, Workspace &workspace) const {
            ...
            if (m_length >= BlockSize && dst.cols() > 1) {
                for (Index i = 0; i < m_length; i += BlockSize) {
                    ...
                    typename internal::conditional<Side ==
                                                   OnTheRight, Transpose<SubVectorsType>, SubVectorsType &>::type sub_vecs(
                            sub_vecs1);
                    Block<Dest, Dynamic, Dynamic> sub_dst(dst, dst.rows() - rows() + m_shift + k, 0,
                                                          rows() - m_shift - k, dst.cols());
                    apply_block_householder_on_the_left(sub_dst, sub_vecs, m_coeffs.segment(k, bs),
                                                        !m_trans);
                }
            } else {
        ...
        Index
        length() const { return m_length; }  /**< \brief Returns the length of the Householder sequence. */
        Index
        shift() const { return m_shift; }    /**< \brief Returns the shift of the Householder sequence. */
